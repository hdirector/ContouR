...
#point on a vertex
test_sq_vert <- ptInPoly(t(sq_poly), c(1, 1))
...
#points inside 1
test_typ_in1 <- ptInPoly(t(typ_poly), c(0, 3.2))
stopifnot(test_typ_in1 == TRUE)


#point to left
test_typ_left <- ptInPoly(t(typ_poly), c(-3, 2.9))
stopifnot(test_typ_left == FALSE)

#point on top 1
test_typ_top1 <- ptInPoly(t(typ_poly), c(-.5, 3.6))
stopifnot(test_typ_top1 == FALSE)

#point on top 2
test_typ_top2 <- ptInPoly(t(typ_poly), c(1.2, 30))
stopifnot(test_typ_top2 == FALSE)

#point on right 
test_typ_right <- ptInPoly(t(typ_poly), c(2.5, 3.4))
stopifnot(test_typ_right == FALSE)

#point on bottom 
test_typ_bottom <- ptInPoly(t(typ_poly), c(1.6, 3.1))
stopifnot(test_typ_bottom1 == FALSE)

#vertex 1
test_typ_ver1 <- ptInPoly(t(typ_poly), c(0.1, 3.5))
stopifnot(test_typ_ver1 == TRUE)

#vertex 2
test_typ_ver2 <- ptInPoly(t(typ_poly),  c(-.25, 2.9))
...
b1 <- 2.7 - m1*(-1)
y_test <- m1*x_test +b1
test_typ_edge1 <- ptInPoly(t(typ_poly), c(x_test, y_test))
stopifnot(test_typ_edge1 == TRUE)

#on edge 2
test_typ_edge2 <-  ptInPoly(t(typ_poly), c(.06, 3.5))
stopifnot(test_typ_edge2 == TRUE)

#on edge 3
test_typ_edge3 <- ptInPoly(t(typ_poly), c(2, 3.85))
...
b1 <- 3.1 - m1*(1.2)
y_test <- m1*x_test + b1
test_typ_edge4 <- ptInPoly(t(typ_poly), c(x_test, y_test))
