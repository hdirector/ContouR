...
mu_true <-
  c(seq(.1, .4, length = p_true / 4), seq(.4, .1, length = 3 * p_true / 4))
...
#result matrix
cover <- matrix(nrow = p_true,
                ncol = length(cred_ints),
                data = 0)
...
for (k in 1:n_evals) {
  ...
  for (i in 1:n_obs) {
    kern_curr <-
      find_kernel(rbind(t(obs_coords[, , i]), t(obs_coords[, 1, i])))
    if (i == 1) {
      kern <- kern_curr
    } else {
  ...
  sigmaPropCov = .01*compSigma(sigma_ini, kappa_ini, theta_dist)
  muPropCov <- .01*compSigma(sigma_ini, kappa_ini, theta_dist)
  
  #run MCMC to get parameter estimates
  start_time <- Sys.time()
  fits <- RunMCMC(nIter = n_iter, x = obs_coords, mu = mu_ini, mu0 = mu0,
                  Lambda0 = Lambda0, muPropCov = muPropCov, nu = .0001,
                  nuPropSD = .001, alphaNu0 = .01, betaNu0 = .1, Cx = C_ini[1], Cx0 = Cx0, 
                  sigmaX0 = sigmaX0, CxPropSD = CxPropSD, Cy = C_ini[2],
                  Cy0 = Cy0, sigmaY0 = sigmaY0, CyPropSD = CyPropSD,
                  kappa = kappa_ini, alphaKappa0 = 0, betaKappa0 = 10,
                  kappaPropSD = kappaPropSD, sigma = sigma_ini,
                  betaSigma0 = betaSigma0, sigmaPropCov = sigmaPropCov, 
  ...
  pts_test <-
    rbind(cbind(x1_test, x2_test), c(x1_test[1], x2_test[1]))
  test_truth <- make_line(p1 = pts_test, name = "test_truth")
  
  #credible intervals and coverage
  prob <-
    get_prob_sim(n_sim, mu_est, Sigma_est, Cx_est, Cy_est, theta_est)
